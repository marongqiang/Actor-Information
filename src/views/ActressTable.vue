<template>
  <div class="actress-table-page">
    <div class="toolbar">
      ...
      <span v-if="selectedRows.length" style="color: #409eff; font-size: 12px;">
        已选 {{ selectedRows.length }} 项
        <el-button size="small" type="warning" @click="batchMerge">批量合并</el-button>
      </span>
    </div>
    ...
    >
      ...
      <el-table-column prop="aliases" label="别名" min-width="120">
        <template #default="{ row }">
      ...
      <el-table-column prop="source" label="来源" width="80">
        <template #default="{ row }">
          <el-tag :type="row.source === 'local_folder' ? 'success' : ''" size="small">
            {{ row.source === 'local_folder' ? '本地' : '网络' }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="三围" width="110">
        <template #default="{ row }">
          <span v-if="row.bust">{{ row.bust }}/{{ row.waist }}/{{ row.hip }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="cup" label="罩杯" width="65" />
      <el-table-column prop="letter" label="首字母" width="65" />
      <el-table-column prop="movie_count" label="作品" width="65" sortable="custom" />
    ...
    <el-dialog v-model="mergeDialog" title="合并演员" width="500px">
      ...
      <el-select v-model="targetActressId" filterable placeholder="搜索目标演员..." style="width: 100%; margin: 12px 0;" size="small">
      ...
      <div style="margin: 8px 0;">
        ...
        <el-checkbox v-model="mergeOptions.dryRun">模拟运行</el-checkbox>
      </div>
      <el-radio-group v-model="mergeOptions.conflictPolicy" size="small">
    ...
    <el-dialog v-model="dupDialog" title="重复演员" width="500px">
      <el-table :data="store.duplicates" size="small">
        <el-table-column prop="id1" label="ID1" width="70" />
        <el-table-column prop="id2" label="ID2" width="70" />
...
const selectedRows = ref<ActressItem[]>([])
const tableRef = ref()
...
function onSortChange(sort: any) {
  store.fetchPaginated(page.value, 20, search.value || undefined, sort.prop, sort.order)
}

async function fetchAliases() {
  // Batch fetch aliases for display
  for (const a of store.actresses) {
    try { (a as any)._aliases = await store.getAliases(a.id) } catch { (a as any)._aliases = [] }
  }
}
...
function showMergeDialog(row: ActressItem) {
  selectedActress.value = row; targetActressId.value = null; mergeResult.value = null; mergeDialog.value = true
}

async function executeMerge() {
  if (!selectedActress.value || !targetActressId.value) { ElMessage.warning('请选择目标演员'); return }
  mergeResult.value = await store.mergeActressesWithOptions(selectedActress.value.id, targetActressId.value, mergeOptions.value)
  if (mergeResult.value?.success) ElMessage.success('合并完成')
}

async function batchMerge() {
  if (selectedRows.value.length < 2) { ElMessage.warning('至少选择2个演员'); return }
  ElMessage.info('批量合并功能待实现')
}
...
async function detectDup() {
  await store.detectDuplicates(); dupDialog.value = true
}
...
.toolbar {
  ...
  margin-bottom: 14px; padding: 10px 14px; background: #1a1a2e; border-radius: 8px;
}
.toolbar h2 { flex: 1; font-size: 16px; min-width: 80px; }
</style>
