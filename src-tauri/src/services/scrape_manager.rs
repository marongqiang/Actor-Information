...
fn scrape_fanza(query: &str) -> Result<ScrapeResult, CommandError> {
    ...
    // Try digital-padded ID: SONE-131 → sone00131 (letters + 5-digit pad)
    let code_digital = if let Some(pos) = code_lower.find(|c: char| c.is_ascii_digit()) {
        let prefix = &code_lower[..pos];
        let num = &code_lower[pos..];
        format!("{}{:0>5}", prefix, num)
