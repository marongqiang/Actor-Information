...
pub fn get_movies_paginated(
    ...
    let query_sql = format!(
        "SELECT m.file_id, m.title, m.original_title, m.chinese_name, m.chinese_overview, m.year, m.poster_local, m.rating, m.genre, m.runtime, m.is_hidden,
    ...
    let rows = stmt.query_map(params_ref.as_slice(), |row| {
        Ok(MovieRow {
            ...
            chinese_overview: row.get(4)?,
            year: row.get(5)?,
            poster_local: row.get(6)?,
            rating: row.get(7)?,
            genre: row.get(8)?,
            runtime: row.get(9)?,
            ...
            progress: row.get(11)?,
            duration: row.get(12)?,
            scrape_status: row.get(13)?,
            scrape_started_at: row.get(14)?,
            scrape_finished_at: row.get(15)?,
            scrape_error: row.get(16)?,
        })
...
pub struct MovieRow {
    ...
    pub chinese_overview: Option<String>,
    pub year: Option<i32>,
    pub poster_local: Option<String>,
    pub rating: Option<f64>,
    pub genre: Option<String>,
...
pub fn get_movie_detail(conn: &Connection, file_id: &str) -> CommandResult<Option<MovieDetailRow>> {
    let result = conn.query_row(
        "SELECT file_id, title, original_title, chinese_name, chinese_overview, year, poster_url, poster_local, backdrop_url,
         overview, rating, runtime, director, genre, file_name, file_size, created_at, updated_at, is_hidden
